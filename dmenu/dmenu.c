...
static unsigned int textw_clamp(const char *str, unsigned int n,
  ...
  if (startswith(image_prefix, str) &&
      (w = drw_getimagewidth_clamp(drw, str + strlen(image_prefix), maxw,
                                   maxh)))
...
static unsigned int texth_clamp(const char *str, unsigned int n,
                                unsigned int maxw, unsigned int maxh) {
  unsigned int h;
  if (startswith(image_prefix, str) &&
      (h = drw_getimageheight_clamp(drw, str + strlen(image_prefix), maxw,
                                    maxh)))
...
static void calcoffsets(void) {
  ...
  for (i = 0, next = curr; next; next = next->right)
    if ((i += (lines > 0) ? texth_clamp(next->text, n, mw - lrpad, image_size)
...
static int drawitem(struct item *item, int x, int y, int w) {
  ...
  if (startswith(image_prefix, item->text)) {
    ...
    drw_image(drw, &x, &y, &image_width, &image_height, lrpad,
              vertical ? tbpad : 0, path, vertical);
...
static void drawmenu(void) {
  ...
  if (lines > 0) {
    ...
    for (item = curr; item != next; item = item->right)
      x = drawitem(
          item, x, 0,
          textw_clamp(item->text, mw - x - TEXTW(">"), image_size, bh));
    if (next) {
      w = TEXTW(">");
      drw_setscheme(drw, scheme[SchemeNorm]);
  ...
  for (i = 1; i < argc; i++)
    ...
    /* these options take one argument */
      ...
      image_prefix = argv[++i];
                     ...
                     "-is")) /* max. image preview size (height or width) */
